CREATE OR REPLACE TEMP VIEW silver_ability_translation_stage AS
...
decoded AS (
 ...
 SELECT payload.id ability_id,CAST(NULLIF(REGEXP_EXTRACT(n.language.url,'/([0-9]+)/?$',1),'') AS BIGINT) language_id,
...
SELECT *,CURRENT_TIMESTAMP() silver_transformed_at,{{run_id}} silver_run_id,
 ability_id IS NOT NULL AND language_id IS NOT NULL AND localized_name IS NOT NULL is_valid,
 CONCAT_WS('; ',CASE WHEN ability_id IS NULL THEN 'null_ability_id' END,CASE WHEN language_id IS NULL THEN 'null_language_id' END,
